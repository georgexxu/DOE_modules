...
for epsilon in [0.02]:#,0.04,0.08,0.16]:
    ...
    for grid_num in [512]:
        ...
        # for dt in [0.9]:
            ...
            figure = True
            write_to_file = True
            filename = "epsilon"+str(epsilon)+"_bdf2_ieq_1st_ieq_"+fine_coarse+".txt"
            ...
            eta_n.assign(phi_0)
            # eta.assign(phi_0) #eta is still as trialfunciton
            ...
            a_start = eta/dt*w*dx + dot(grad(eta),grad(w))*dx \
                + 1/(epsilon*epsilon)*H(eta_n)*H(eta_n)*1.0/2*eta*w*dx
            L_start = eta_n/dt*w*dx - 1/(epsilon*epsilon)*H(eta_n)*H(eta_n)*(-1.0/2)*eta_n*w*dx \
                - 1/(epsilon*epsilon)*H(eta_n)*func_U(eta_n)*w*dx
            ...
            solver1.set_from_options()
            A, b = assemble_system(a_start, L_start)
            solver1.set_operator(A)
            eta = Function(V1)
            solver1.solve(eta.vector(), b)
            eta_n.assign(eta)
            counter = 1
            t += dt 
            ...
            solutions.append(eta.vector().get_local()[::sol_space]) #store first dt solution
            Rs = [] 
            ts = []
            a1 = time.time()
