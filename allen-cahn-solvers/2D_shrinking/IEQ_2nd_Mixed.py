...
for epsilon in [0.02]:#,0.04,0.08,0.16]:
    ...
    for grid_point in [512]:
        for dt in [0.002]: #,0.3,0.4,0.5,0.6,0.7,0.8,0.9,1.0]:
            ...
            #======== Output parameters======
            write_file_name1 = "mixed_linear_2D_radius_"+str(epsilon)+'.txt'
            write_file_name2 = "mixed_linear_2D_ts_"+str(epsilon)+'.txt'
            fig_folder_name = "solution"+str(epsilon)
            write_frequency = 1
            write_to_file = True 
            save_figure = True
            ...
            a = 3*phi/(2*dt)*v1*dx + dot(grad(phi),grad(v1))*dx \
                + 1/(epsilon*epsilon)*H(2*phi_n - phi_n_1)*U*v1*dx \
                + 3*U*v2*dx - 1.0/2*H(2*phi_n - phi_n_1)*(3*phi)*v2*dx + a_stab
            L_stab = S/(epsilon*epsilon)*(2*phi_n - phi_n_1)*v1*dx 
            # L_stab = 0
            L = -(- 4*phi_n +phi_n_1)/(2*dt)*v1*dx - (- 4*func_U(phi_n) + func_U(phi_n_1))*v2*dx \
                + 1.0/2*H(2*phi_n - phi_n_1)*(- 4*phi_n +phi_n_1)*v2*dx + L_stab
